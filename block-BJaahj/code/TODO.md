...
function objOfMatchesWithArray(array1, array2, callback) {
  ...
  let newArr = [];
  array1.forEach((x) => {
    return newArr.push(callback.reduce((acc,cv) => {
      return acc = cv(acc);
    }, x));
  })
  for(let k = 0; k < array2.length;k++){
    if(newArr[k] === array2[k])
    {
    obj[array1[k]] = array2[k];
    }
  }
  return obj;
...
```js
function schedule(string,tym) {
  if(string.length === tym.length){
    for(let i =0; i< string.length;i++){
      setTimeout(string[i], tym[i] * 1000);
    }
  }
  else{
    alert(`invalid input`);
